...
function Swap({ address, isConnected }) {
  ...
  // to fetch the prices to backend from moralis API
  useEffect(() => {
    fetchPrices(tokenList[0].address, tokenList[1].address);
  }, []);

  // to fire sendTransaction() from wagmi-lib
  useEffect(() => {
  ...
  // to show message/modal, show user the transaction pending status
  useEffect(() => {
    messageApi.destroy();
  ...
  // to show message/modal, show user the transaction success/failed status
  useEffect(() => {
    messageApi.destroy();
    if (isSuccess) {
      messageApi.open({
